...
integer(hid_t) , dimension(8) :: conc_dspace_id
integer(hsize_t) , dimension(1) , parameter :: conc_init_dims = (/1/)
integer(hsize_t) , dimension(2) :: conc1d_init_dims
integer(hsize_t) , dimension(3) :: conc2d_init_dims
integer(hid_t) , dimension(8) :: conc_crp_list
...
integer(hid_t) , dimension(8) :: conc_dset_id
! Solvent group
...
integer(hid_t) , dimension(78) :: solv_dspace_id
integer(hsize_t) , dimension(2) :: solvent_init_dims
integer(hid_t) , dimension(8) :: solv_crp_list
...
integer(hid_t) , dimension(8) :: solv_dset_id
! Dimer group
...
subroutine output_hdf5_initiate(cstep,Freq1,Freq2,Freq3,Freq4,box,T,MDt,MPCt,NT,numdims)
implicit none
...
conc2d_init_dims = (/int(box(1)*2.),int(box(2)*2.),1/)
...
do i = 1,8,1
 ...
 call h5screate_simple_f(rank(3),conc2d_init_dims,conc_dspace_id(i),err,maxdims_3)
 call h5pcreate_f(h5p_dataset_create_f,conc_crp_list(i),err)
...
call h5awrite_f(dimer_attr_id,h5t_native_integer,Freq1,dimer_attr_1_dims,err)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine output_hdf5_write(cstep,Vol,Freq1,Freq2,Freq3,Freq4,NT,numdims,Ma,Mb,Mc,Mn,zdimeq,box,x_width,y_width,&
...
real(kind=dbl) , intent(in) :: zdimeq
real(kind=dbl) , dimension(3) , intent(in) :: box
...
integer(kind=sgl) , dimension(:) , intent(in) :: type_B
...
real(kind=dbl) , dimension(:,:) , intent(inout) :: concA2d
real(kind=dbl) , dimension(:,:) , intent(inout) :: concB2d
...
integer(hsize_t) , dimension(3) :: conc2d_extend
integer(hsize_t) , dimension(3) :: conc2d_extend_offset
integer(hsize_t) , dimension(3) :: conc2d_extend_count
...
if ( mod(cstep,Freq1) == 0 ) then
 ...
 call h5dwrite_f(solv_dset_id(5),h5t_native_integer,type_B,solvent_init_dims,err)
 call h5dwrite_f(solv_dset_id(6),h5t_native_double,vxsol,solvent_init_dims,err)
 call h5dwrite_f(solv_dset_id(7),h5t_native_double,vysol,solvent_init_dims,err)
 call h5dwrite_f(solv_dset_id(8),h5t_native_double,vzsol,solvent_init_dims,err)
...
do i = 1,NT
 if ( zsol(i) <= ( zdimeq + cutoffC ) .and. zsol(i) >= ( zdimeq - cutoffC ) ) then
...
! 2D concentration (along x/y axis; 2 removed from upper nad lower part of z BB
! boundary
concA2d = 0.
concB2d = 0.

vol2d = x_width*y_width*( 2.*cutoffC  )
do i = 1,NT
 if ( zsol(i) <= ( zdimeq + cutoffC ) .and. zsol(i) >= ( zdimeq - cutoffC ) ) then
  j = ceiling(xsol(i)/x_width)
  k = ceiling(ysol(i)/y_width)
  if ( flag(i) == 0 ) then
   concA2d(j,k) = concA2d(j,k) + 1.
  else if ( flag(i) == 1 ) then
   concB2d(j,k) = concB2d(j,k) + 1.
  end if
 end if
end do

concA2d = concA2d / vol2d
concB2d = concB2d / vol2d
...
conc2d_init_dims = (/int(box(1)/x_width),int(box(2)/y_width),1/)
...
call h5dwrite_f(conc_dset_id(7),h5t_native_double,concA2d,conc2d_init_dims,err)
call h5dwrite_f(conc_dset_id(8),h5t_native_double,concB2d,conc2d_init_dims,err)
...
conc2d_extend = (/int(box(1)/x_width),int(box(2)/y_width),int(cstep/Freq2)+1/)
conc2d_extend_offset = (/0,0,int(cstep/Freq2)/)
conc2d_extend_count = (/int(box(1)/x_width),int(box(2)/y_width),1/)
...
do i = 1,8,1
 if ( i > 0 .and. i <= 4 ) then
  ...
  call h5dset_extent_f(conc_dset_id(i),conc2d_extend,err)
  call h5screate_simple_f(rank(3),conc2d_extend_count,concentration_memspace,err)
  call h5dget_space_f(conc_dset_id(i),conc_dspace_id(i),err)
  call h5sselect_hyperslab_f(conc_dspace_id(i),h5s_select_set_f,conc2d_extend_offset,conc2d_extend_count,err)
 end if
 if ( i == 1 ) then             ! TotA
  ...
  call h5dwrite_f(conc_dset_id(i),h5t_native_double,concA2d,conc2d_extend_count,err,concentration_memspace,conc_dspace_id(i))
 else if ( i == 8 ) then       ! ConcB2d
  call h5dwrite_f(conc_dset_id(i),h5t_native_double,concB2d,conc2d_extend_count,err,concentration_memspace,conc_dspace_id(i))
 end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine calc_solv_type_num(NT,Vol,flag,iTotA,iTotB)
implicit none
