...
subroutine sort_hilbert(vol,NT,box,xsol,ysol,zsol,flag,vxsol,vysol,vzsol)
implicit none
...
allocate(tcelln(vol),scelln(vol,cell_max),newvol(vol),newbox(3),newxsol(NT),&
         newysol(NT),newzsol(NT),newflag(NT),newvxsol(NT),&
...
!$OMP SHARED(NT,xsol,ysol,zsol,flag,vxsol,vysol,vzsol) &
!$OMP SHARED(newxsol,newysol,newzsol,newflag,newvxsol,newvysol,newvzsol) &
!$OMP PRIVATE(i)
do i = 1,NT
 xsol(i) = newxsol(i)
 ysol(i) = newysol(i)
...
deallocate(tcelln,scelln,newvol,newbox,newxsol,newysol,newzsol,newflag,newvxsol,&
         newvysol,newvzsol)
