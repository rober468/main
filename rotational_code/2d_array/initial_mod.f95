...
subroutine initial(Box,numdims,Dcn,Na0,Nb0,NT,cutoff,Rc,Rn,zdimeq,varA,varB,mean,flag,Xdim,Ydim,Zdim, &
                   ...
                   FpairX,FpairY,FpairZ,motorCM,sep,numrows,numcols)
use mt95
use gauss_rand_mod
implicit none
...
real(kind=dp) , intent(in) :: numrows
real(kind=dp) , intent(in) :: numcols
...
do j = 1,int(numrows)
 do k = 1,int(numcols)
  ...
  theta = 2. * pi * theta
  i = i + 1
  Xdim(i,1) = 0.5*(box(1) - (Dcn+2.*Rc*cutoff)*numcols - sep*(numcols-1) ) + 0.5*(Dcn+2.*Rc*cutoff) + &
              (k-1)*(Dcn+2.*Rc*cutoff+sep) + 0.5*Dcn*cos(theta) 
  Ydim(i,1) = 0.5*(box(2) - (Dcn+2.*Rc*cutoff+sep)*(numrows-1) - (Dcn+2.*Rc*cutoff) ) + &
              0.5*(Dcn+2.*Rc*cutoff) + (j-1)*(Dcn+2.*Rc*cutoff+sep) + 0.5*Dcn*sin(theta)
  Zdim(i,1) = zdimeq
  Xdim(i,2) = 0.5*(box(1) - (Dcn+2.*Rc*cutoff)*numcols - sep*(numcols-1) ) + 0.5*(Dcn+2.*Rc*cutoff) + &
              (k-1)*(Dcn+2.*Rc*cutoff+sep) - 0.5*Dcn*cos(theta)
  Ydim(i,2) = 0.5*(box(2) - (Dcn+2.*Rc*cutoff+sep)*(numrows-1) - (Dcn+2.*Rc*cutoff) ) + &
              0.5*(Dcn+2.*Rc*cutoff) + (j-1)*(Dcn+2.*Rc*cutoff+sep) - 0.5*Dcn*sin(theta)
  Zdim(i,2) = zdimeq
