...
do i = 1,NT
 tCellshift(NewCellN(i)) = tCellshift(NewCellN(i)) + 1
 !Sum the velocities in each new cell N; used next for center of mass velocity calc.
 VXCM(NewCellN(i)) = VXCM(NewCellN(i)) + VXsol(i)
 VYCM(NewCellN(i)) = VYCM(NewCellN(i)) + VYsol(i)
 VZCM(NewCellN(i)) = VZCM(NewCellN(i)) + VZsol(i)
end do
...
do i = 1,numcell
 ...
 phi = 2.d0*pi*x                                !Random angle between 0 and 2*pi
 theta = 2.d0*y-1.d0
