...
metadata = dict(title="rmpc_B>A_1")
...
animation = anim.FuncAnimation(fig,animate,init_func=init,frames=len(dim_x[:,0,0]),interval=10,blit=True)
animation.save("motors_dynamics.mp4", writer=ffwriter,fps=int(len(dim_x[:,0,0])),dpi=300)
plt.close(fig)
