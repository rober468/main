...
subroutine LJ_mot_sol(sphere,NT,Box,flag,Xsol,Ysol,Zsol,Xdim,Ydim,Zdim, &
                    FpairX,FpairY,FpairZ,Potential,RPab,RPba,cutoff2,Ea,Eb,Es,R6,R12,r12R12,r6R6, &
...
real(kind=dp) , intent(in) :: Es                                ! LJ pot. with monomer
...
!$OMP SHARED(R6,Ea,Eb,Es,RPab,RPba,sphere,flag,neighbour_num,neighbour) &
!$OMP SHARED(FpairX,FpairY,FpairZ,Potential) &
!$OMP SCHEDULE(static)
...
do i = 1,neighbour_num
 ...
 if ( dRsq <= cutoff2 ) then                           ! If within cutoff2 region of LJ pot.
  ...
  if ( flag(neighbour(i)) == 0 ) then                  ! For A molecules:
  ...
  else if ( flag(neighbour(i)) == 2 ) then             ! If molecule is S, then:
   IdRsq = 1.d0/dRsq                                   ! Inverse of the square of dR
   IdR6 = IdRsq**3                                     ! Inverse of (dR)**6
   IdR8 = IdR6 * IdRsq                                 ! Inverse of (dR)**8
   IdR12 = IdR6 * IdR6                                 ! Inverse of (dR)**12
   ...
   Fbrack = 4.d0 * ( r12R12*IdR14-r6R6*IdR8 )          ! LJ force calc., part with radii
   Ubrack = 4.d0*(R12*IdR12-R6*IdR6)+1.d0              ! LJ pot. calc., part with radii
   FpairX(i) = Es * Fbrack * dR(1)                     ! LJ force in x direction
   FpairY(i) = Es * Fbrack * dR(2)                     ! LJ force in y direction
   FpairZ(i) = Es * Fbrack * dR(3)                     ! LJ force in z direction
   Potential(i) = Potential(i) + Es * Ubrack           ! LJ Potential 
...
end subroutine LJ_mot_sol

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

subroutine LJ_mot_mot(box,Xdim1,Ydim1,Zdim1,Xdim2,Ydim2,Zdim2, &
                      FmotX,FmotY,FmotZ,Fmot_pot,cutoff2,Em,R6,R12,r6R6,r12R12)
implicit none
...
! Definitions for in/out variables
real(kind=dp) , dimension(:) , intent(in) :: box                ! Length of simulation box
real(kind=dp) , intent(in) :: Xdim1
real(kind=dp) , intent(in) :: Ydim1
real(kind=dp) , intent(in) :: Zdim1
real(kind=dp) , intent(in) :: Xdim2
real(kind=dp) , intent(in) :: Ydim2
real(kind=dp) , intent(in) :: Zdim2
real(kind=dp) , intent(out) :: FmotX           ! LJ force in x direction
real(kind=dp) , intent(out) :: FmotY           ! LJ force in y direction
real(kind=dp) , intent(out) :: FmotZ           ! LJ force in z direction
real(kind=dp) , intent(out) :: Fmot_pot        ! Potential of each particle
real(kind=dp) , intent(in) :: cutoff2                           ! LJ cutoff for monomer
real(kind=dp) , intent(in) :: Em                                ! LJ pot. with monomer
real(kind=dp) , intent(in) :: R6                                ! R^6
real(kind=dp) , intent(in) :: R12                               ! R^12
real(kind=dp) , intent(in) :: r12R12                            ! 12*R^12
real(kind=dp) , intent(in) :: r6R6                              ! 6*R^6
...
real(kind=dp) :: Ubrack                                         ! LJ pot. calc., part with radii
 ...
 if ( dRsq <= cutoff2 ) then                                    ! If within cutoff2 region of LJ pot.
  ...
  Fbrack = 4.d0 * ( r12R12*IdR14-r6R6*IdR8 )                   ! LJ force calc., part with radii
  Ubrack = 4.d0*(R12*IdR12-R6*IdR6)+1.d0                       ! LJ pot. calc., part with radii
  FmotX = Em * Fbrack * dR(1)                                  ! LJ force in x direction
  FmotY = Em * Fbrack * dR(2)                                  ! LJ force in y direction
  FmotZ = Em * Fbrack * dR(3)                                  ! LJ force in z direction
  Fmot_pot = Em * Ubrack                                       ! LJ Potential  
 end if

end subroutine LJ_mot_mot

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

subroutine LJ_force_wall(sphere,box,zdim,fwallz,Ew,R3,R9,r3R3,r9R9,Potential,k)
implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine LJ_mot_wall(box,zdim,fwallz,Ew,R3,R9,r9R9,r3R3,cutoff2,potential)
implicit none
...
real(kind=dp) , intent(in) :: r9R9                              ! 9*R^9
real(kind=dp) , intent(in) :: r3R3                              ! 3*R^3
real(kind=dp) , intent(in) :: cutoff2                           ! Cutoff for mot-wall int., squared
real(kind=dp) , intent(inout) :: potential                      ! Potential of each particle

! Definitions for internal variables
integer(kind=dp) :: i                                           ! Loop variable
real(kind=dp) :: pref                                           ! prefactor; 0.5*3**1.5
real(kind=dp) :: dR                                             ! wall-monomer distance
real(kind=dp) :: IdRsq                                          ! Inverse of the square of dRsq
real(kind=dp) :: IdR3                                           ! Inverse of (dR)**6
real(kind=dp) :: IdR9                                           ! Inverse of (dR)**8
...
pref = 0.5*3.*sqrt(3.d0)

! Calculate force on upper wall of z-axis from monomer
dR = box(3) - zdim

if ( dR**2 < cutoff2 ) then
 IdRsq = 1. / (dR**2)
 IdR3 = IdRsq ** (3./2.)
 IdR9 = IdR3 ** 3
 IdR5 = IdR3 * IdRsq
 ...
 Fbrack = pref*Ew*(r9R9*IdR11-r3R3*IdR5)
 fwallz = Fbrack * dR
 Potential = pref*Ew*(R9*IdR9-R3*IdR3)+1.d0
end if
...
if ( dR**2 < cutoff2 ) then
 ...
 Fbrack = pref*Ew*(r9R9*IdR11-r3R3*IdR5)
 fwallz = fwallz + Fbrack * dR
 Potential = Potential + pref*Ew*(R9*IdR9-R3*IdR3)+1.d0
end if

end subroutine LJ_mot_wall
